...
|---|---|---|
| Python | 3.10+ | 3.12 recommended; uses `X \| Y` type syntax |
...
cd <team-repo>/part4_storage_alerting
...
Do this in **week one**, not at the end — the walking skeleton finds interface
problems while they are still cheap.

- [ ] Everyone exports the same `HONEYPOT_DB_PATH`
- [ ] Part 4 runs `python main.py init`
- [ ] Part 1 sends one heartbeat → Part 2 → Part 4 → `nodes.status = 'online'`
- [ ] Part 5 renders that one node
- [ ] Parts 1 and 2 import `db/validation.py` instead of hand-rolling checks
- [ ] Part 1 switches to `deterministic_event_id()` and namespaced session IDs
- [ ] Team agrees on the `errors` array addition to the collector response
- [ ] Part 3 runs against the shared fixture IPs and writes reputation rows
- [ ] Full path verified: real attack → alert → feed → dashboard
...
## 8. Team Contract Notes

Five issues in Baseline v1.3 that Part 4 works around, all needing group
agreement to fix properly:

1. **Cowrie session IDs are not unique across nodes.** They are short hex
   strings, and `session_id` is a primary key. Namespace them at the shipper:
   `node-01:a1b2c3d4`.
2. **Random `event_id`s break the deduplication rule.** Use
   `deterministic_event_id()` so a retry after a crash produces the same ID.
3. **Successful logins lose the password.** `login_success` allows only
   `username` in `details`, so a cracked credential pair is never recorded and
   the brute-force counter misses the attempt that actually worked. Fix without
   touching the contract: emit a `login_attempt` for successes too, then the
   `login_success` immediately after.
4. **The `commands` table vanished.** The mentor's plan tells Part 2 to write to
   `sessions` and `commands`; v1.3 folded commands into `events.details`. The
   `commands` view in `schema.sql` satisfies both documents with no duplicated
   storage.
5. **`reputation.source` is one column but there are two sources.** It is TEXT,
   so `upsert_reputation()` accumulates: `"GeoLite2,AbuseIPDB"`. No schema change
   needed — the team just needs to agree the separator is a comma.

**Change control:** table names, column names, allowed event types, the `details`
contract and endpoint behaviour are frozen. Anything under "DERIVED OBJECTS" in
`schema.sql` — views and indexes — is additive and safe to change.
